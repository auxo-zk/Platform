...
async function main() {
  ...
  if (isFunding) {
    ...
    await compile(ZkApp.Request.RequestContract, cache, logMemory, profiler);
  ...
  if (isDeploy) {
    ...
    tx = await Mina.transaction(
      ...
      () => {
        ...
        treasuryContract.zkApps.set(treasuryAddressStorage.addresses.getRoot());
        feePayerAccount.send({
  ...
  if (isFunding) {
    ...
    for (let i = 0; i < investors.length; i++) {
      ...
      tx = await Mina.transaction(
        { sender: investors[i].publicKey, fee, nonce: ++investorNonce[i] },
        () => {
  ...
  if (isTreasury) {
    ...
    let claimFundInput = [
      new ClaimFundInput({
        ...
        participationIndexWitness: indexStorage.getLevel1Witness(Field(1)),
        ...
        participationRef: getZkAppRef(
          treasuryAddressStorage.addresses,
      ...
      new ClaimFundInput({
        ...
        DWitness: DStorage.getWitness(Field(6969)),
        investVector: investVectors,
        participationIndexWitness: indexStorage.getLevel1Witness(Field(3)),
        claimedIndex: claimedStorage.getLevel1Witness(
          claimedStorage.calculateLevel1Index({
            campaignId: Field(1),
            projectId: Field(2),
