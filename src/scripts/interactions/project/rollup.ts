  ...
  projects.map((project: any) => {
    if (Boolean(project.active)) {
      ...
      projectInfoStorage.updateLeaf(
        projectInfoStorage.calculateLeaf(IPFSHash.fromString(project.ipfsHash)),
        Field(project.projectId)
      );
      payeeStorage.updateLeaf(
        payeeStorage.calculateLeaf(PublicKey.fromBase58(project.payeeAccount)),
        Field(project.projectId)
      );
    }
  });
  ...
  for (let i = 0; i < reduceActions.length; i++) {
    ...
    proof = await CreateProject.nextStep(
      ...
      projectInfoStorage.getLevel1Witness(
        projectInfoStorage.calculateLevel1Index(Field(nextProjectId + i))
      ),
