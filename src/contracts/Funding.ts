...
class FundingAction extends Struct({
    ...
    static fromFields(fields: Field[]): FundingAction {
        return super.fromFields(fields) as FundingAction;
    }
...
class FundingContract extends SmartContract {
    ...
    @method async rollup(rollupFundingProof: RollupFundingProof) {
        ...
        nextFundingId.assertEquals(
            rollupFundingProof.publicOutput.initialFundingId
        );
        fundingInformationRoot.assertEquals(
            rollupFundingProof.publicOutput.initialFundingInformationRoot
        );
        ...
        this.nextFundingId.set(rollupFundingProof.publicOutput.nextFundingId);
        this.fundingInformationRoot.set(
            rollupFundingProof.publicOutput.nextFundingInformationRoot
        );
...
class FundingContractMock extends SmartContract {
    @state(Field) nextFundingId = State<Field>();
    @state(Field) fundingInformationRoot = State<Field>();
    @state(Field) zkAppRoot = State<Field>();
    @state(Field) actionState = State<Field>();
